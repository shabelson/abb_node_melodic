...
bool RobotController::init()
{
  ...
  //const std::string msg = robotIp + ": Robot IP";
...
bool RobotController::defaultRobotConfiguration()
{
  ...
  {
    ROS_DEBUG("SET WORK OBJECT PROBLEM");
    return false;
  }
  //Tool
  ROS_INFO("TOOL");
  ...
  ///////SET VACUM =>  I Suppose there is a missing SET DIO in the RAPID file.  TODO!
  //Vacuum
  //ROS_INFO("VACUM");
...
void RobotController::advertiseTopics()
{
  ...
  handle_CartesianLog =node->advertise<geometry_msgs::PoseStamped>("pose", 100);
  handle_JointsLog = node->advertise<sensor_msgs::JointState>("jointstate", 100);
  handle_ForceLog =node->advertise<geometry_msgs::WrenchStamped>("wrench", 100);
    ...
    double q0, double qx, double qy, double qz)
{
  ...
  if(x!=curToolP[0] || y!=curToolP[1] || y!=curToolP[2] || 
	...
	return false;
      ...
      char reply[MAX_BUFFER];
      int msgCode = 06;
      strcpy(message, abb_comm::setTool(x, y, z, q0, qx, qy, qz).c_str());
      ROS_INFO(message);
      if(sendAndReceive(message, strlen(message), reply, msgCode))
	{
...
bool RobotController::setWorkObject(double x, double y, double z, 
    double q0, double qx, double qy, double qz)
{
  ROS_INFO("ENTERED SET WORK OBJECT");
  // Only take action if the required values are different than the actual ones
...
bool RobotController::setSpeed(double tcp, double ori)
{
  ...
  if(tcp!=curSpd[0] || ori!=curSpd[1])
	...
	return false;
      ...
      char reply[MAX_BUFFER];
      int  msgCode = 8;
      strcpy(message, abb_comm::setSpeed(tcp, ori,msgCode).c_str());
      if(sendAndReceive(message, strlen(message), reply, msgCode))
	{
...
bool RobotController::setVacuum(int v)
{
  ...
  if(v!=curVacuum)
      ...
      if((v != VACUUM_OPEN) && (v != VACUUM_CLOSE))
	{
      ...
      strcpy(message, abb_comm::setVacuum(v, msgCode).c_str());
...
void RobotController::advertiseServices()
{
  ...
  handle_Stop = node->advertiseService("Halt",&RobotController::robot_Stop, this);
  handle_SetTool = node->advertiseService("SetTool",&RobotController::robot_SetTool, this);
  handle_SetWorkObject = node->advertiseService("SetWorkObject",&RobotController::robot_SetWorkObject, this);
  handle_SetSpeed = node->advertiseService("SetSpeed",&RobotController::robot_SetSpeed, this);
  handle_SetZone = node->advertiseService("SetZone",&RobotController::robot_SetZone, this);
  handle_SetTrackDist = node->advertiseService("SetTrackDist",&RobotController::robot_SetTrackDist, this);
  handle_SetComm = node->advertiseService("SetComm",&RobotController::robot_SetComm, this);
  handle_SpecialCommand = node->advertiseService("SpecialCommand", &RobotController::robot_SpecialCommand, this);
  handle_SetVacuum = node->advertiseService("SetVacuum",&RobotController::robot_SetVacuum, this);
  handle_SetDIO = node->advertiseService("SetDIO",&RobotController::robot_SetDIO, this);
  handle_IsMoving = node->advertiseService("IsMoving",&RobotController::robot_IsMoving, this);
}
...
int main(int argc, char** argv)
{
  ...
  const std::string rosNodeName =  "robot_controller";
  ...
  pthread_mutex_init(&nonBlockMutex, nullptr);
  pthread_mutex_init(&jointUpdateMutex, nullptr);
  pthread_mutex_init(&cartUpdateMutex, nullptr);
  pthread_mutex_init(&forceUpdateMutex, nullptr);
  pthread_mutex_init(&sendRecvMutex, nullptr);
